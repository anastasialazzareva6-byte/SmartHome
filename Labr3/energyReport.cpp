...
std::vector<Device*> EnergyReport::getTopConsumingDevices(int count) const {
    std::vector<std::pair<Device*, double>> devices;
    for (const auto& pair : deviceConsumptions) {
...
void EnergyReport::addDeviceConsumption(Device* device, double consumption) {
...
EnergyReport* EnergyReport::deserialize(const std::string& data) {
    ...
    EnergyReport* report = new EnergyReport(id, std::stol(startStr), std::stol(endStr));
    // Âîññòàíîâëåíèå äàííûõ ïîòðåáëåíèÿ 
    return report;
}