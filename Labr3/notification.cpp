...
void Notification::send() {
    ...
    case NotificationType::INFO: std::cout << "ИНФОРМАЦИЯ 🔵"; break;
    }
...
void Notification::markAsRead() {
    isRead = true;
    std::cout << "Уведомление помечено как прочитанное: " << message << std::endl;
}

int Notification::getNotificationPriority() const {
    switch (type) {
    case NotificationType::ALERT: return 1;
    case NotificationType::WARNING: return 2;
    case NotificationType::INFO: return 3;
    default: return 3;
    }
}

void Notification::displayInfo() const {
    std::cout << "[" << (isRead ? "ПРОЧИТАНО" : "НОВОЕ") << "] ";
    std::cout << message << " (";
    switch (type) {
    case NotificationType::ALERT: std::cout << "Тревога"; break;
    case NotificationType::WARNING: std::cout << "Предупреждение"; break;
...
void Notification::setRelatedDevice(Device* device) {
    relatedDevice = device;
}

void Notification::setRelatedScenario(AutomationScenario* scenario) {
...
Notification* Notification::deserialize(const std::string& data) {
    ...
    Notification* notification = new Notification(id, type, message);
